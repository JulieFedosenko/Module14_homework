...
// Проверка числа на соответствие диапазону и является ли оно числом
...
function handleRequest() {
  ...
  // Если номер страницы и лимит не в диапазоне или не являются числами
  if (!isPageValid && !isLimitValid) {
    imageGallery.innerHTML = 'Номер страницы и лимит вне диапазона от 1 до 10';
    return;
  }
  // Если номер страницы не в диапазоне или не является числом
  if (!isPageValid) {
    imageGallery.innerHTML = 'Номер страницы вне диапазона от 1 до 10';
    return;
  }
  // Если лимит не в диапазоне или не является числом
  if (!isLimitValid) {
